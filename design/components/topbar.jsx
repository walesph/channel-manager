...
// Topbar — page title, breadcrumb, channel sync row, lang toggle.
const SyncDot = ({ status }) => {
  const cls = status === 'synced' ? 'ok' : status === 'syncing' ? 'info' : status === 'delayed' ? 'warn' : 'bad';
  return <span className={`pill dot ${cls}`} style={{height: 18, fontSize: 10}}>{status}</span>;
};

const ChannelStrip = ({ channels = CHANNELS, lang = 'ko' }) => {
  const t = STR[lang];
  return (
    <div className="ch-strip">
      ...
      <style>{`
        .ch-strip { display: flex; gap: 6px; align-items: center; }
        .ch-pill {
...
const Topbar = ({ title, sub, breadcrumb, actions, lang = 'ko', onLang, dark, onDark, showSync = true }) => {
  return (
    <header className="topbar">
      <div className="tb-row">
        ...
        <div className="tb-right">
          ...
          <button className="btn ghost lang" onClick={onLang}>
            <I.globe size={13}/>
            <span>{lang === 'ko' ? 'KO' : 'EN'}</span>
          </button>
          <button className="btn ghost icon"><I.bell size={15}/></button>
        </div>
      </div>
      ...
      {showSync && (
        <div className="tb-sync">
          ...
          <div className="sync-meta">
            ...
            <button className="btn sm ghost"><I.refresh size={12}/> {lang === 'ko' ? '지금 동기화' : 'Sync now'}</button>
          </div>
        </div>
      )}
